...
function AddComponent() {
	...
	const validationSchema = Yup.object({
		typeId: Yup.string().required("Vui lòng chọn loại dịch vụ"),
		title: Yup.string().trim().required("Tên phòng không được để trống"),
		size: Yup.number().typeError("Diện tích phải là số").required("Vui lòng nhập diện tích phòng"),
		information: Yup.object({
			bedroom: Yup.number().typeError("Số phòng ngủ phải là số").min(1, "Phải lớn hơn 0").required("Nhập số lượng phòng ngủ"),
			bed: Yup.number().typeError("Số giường ngủ phải là số").min(1, "Phải lớn hơn 0").required("Nhập số lượng giường ngủ"),
			bathroom: Yup.number().typeError("Số phòng tắm phải là số").min(1, "Phải lớn hơn 0").required("Nhập số lượng phòng tắm"),
			kitchen: Yup.number().typeError("Số phòng bếp phải là số").min(1, "Phải lớn hơn 0").required("Nhập số lượng phòng bếp"),
			customer: Yup.number().typeError("Số khách hàng phải là số").min(1, "Phải lớn hơn 0").required("Nhập số lượng khách hàng"),
			price: Yup.string()
							...
							<div className="mb-3">
								...
								{previewImg && (
									<Card.Img
										...
										className="mt-3 max-h-[400px] object-contain"
										alt="imgSrc"
									/>
								)}
							</div>
