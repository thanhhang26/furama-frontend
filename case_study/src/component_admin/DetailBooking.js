...
function DetailBooking() {
	...
	useEffect(() => {
		...
		const detailData = async () => {
			...
			const matchedFacility = facilitiesData.find((facility) => String(facility?.id ?? facility?.facility?.id) === String(detail.facilityId));
	...
	return (
		<div className="container mt-4">
				...
				<Row className="g-5">
					<Col md={6}>
						<div className="mb-3">
							...
							<input type="text" name="customer.lastName" className="form-control" value={bookingDetail.customer.lastName} readOnly />
						</div>
						<div className="mb-3">
							<label className="form-label fw-semibold">Email</label>
							<input type="email" name="customer.email" className="form-control" value={bookingDetail.customer.email} readOnly />
						</div>
					</Col>
					...
					<Col md={6}>
						<div className="mb-3">
							...
							<input type="text" name="customer.firstName" className="form-control" value={bookingDetail.customer.firstName} readOnly />
						</div>

						<div className="mb-3">
							<label className="form-label fw-semibold">Số điện thoại</label>
							<input type="text" name="customer.phone" className="form-control" value={bookingDetail.customer.phone} readOnly />
						</div>
					</Col>
				</Row>
			</div>
				...
				<Row className="g-5">
					...
					<Col md={6}>
						...
						<div className="mb-3">
							...
							<input type="text" name="facilityTitle" className="form-control" value={bookingDetail.facilityTitle} readOnly />
						</div>
					</Col>
				</Row>
			</div>
			...
			<div className="text-end mt-4">
				<Link type="button" className="btn btn-outline-secondary ms-3 mb-3 px-4" to={`/bookingAdmin`}>
					Trở về
				</Link>
			</div>
		</div>
